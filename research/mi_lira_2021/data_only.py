...
def get_data(seed, exclude_class=None, include_one_example=False):
    ...
    if FLAGS.num_experiments is not None:
        ...
        if FLAGS.expid < FLAGS.num_experiments - 1:
    ...
    return train, test, xs, ys, keep, nclass, inputs, labels

def save_images(inputs, labels, keep, save_dir):
    """
    Save individual training images for which keep=1.
    The filename format is "label_ID.png".
    
    Args:
        inputs: Array of input images with values in range [-1, 1]
        labels: Array of labels
        keep: Boolean array indicating which images to keep
    ...
    # Get indices where keep=1
    keep_indices = np.where(keep)[0]
    
    print(f"Saving {len(keep_indices)} images to {images_dir}")
    
    # Save each image
    for idx, original_idx in enumerate(keep_indices):
        # Get image and label
        img = inputs[original_idx]
        label = labels[original_idx]
        
        # Convert from [-1, 1] to [0, 255]
        img = ((img + 1) * 127.5).astype(np.uint8)
        
        # If image is in NCHW format, convert to NHWC
        if img.shape[0] == 3:  # Channel is first dimension
        ...
        # Save image
        Image.fromarray(img).save(os.path.join(images_dir, filename))
...
def main(argv):
    ...
    train, test, xs, ys, keep, nclass, all_inputs, all_labels = get_data(seed, FLAGS.exclude_class)
    ...
    # Save individual images for which keep=1
    save_images(all_inputs, all_labels, keep, logdir)
